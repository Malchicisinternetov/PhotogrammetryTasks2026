...
cv::Vec3d phg::Calibration::unproject(const cv::Vec2d &pixel) const
{
    ...
    double cur_y = y;
    for (int i = 0; i < 10; i++) {
        double r = cur_x*cur_x + cur_y*cur_y;
        double L = 1.0 + k1_ * r + k2_ * r*r;
        cur_x = x / L;
        cur_y = y / L;
    }
    ...
    // мы не можем напрямую обратить, так как в формуле участвует полином и мы не можем просто извлечь корень, поэтому используем итерационный подход,
    // 10 итераций достаточно чтобы добиться субпиксельной точности
    return cv::Vec3d(x, y, 1.0);
}
