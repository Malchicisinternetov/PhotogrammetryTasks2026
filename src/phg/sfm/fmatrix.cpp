...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        const Eigen::MatrixXd& U = svdf.matrixU();
        const Eigen::VectorXd& s = svdf.singularValues();
        const Eigen::MatrixXd& V = svdf.matrixV();
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        cx /= static_cast<int>(m.size());
        cy /= static_cast<int>(m.size());

        double av_dist = 0;
        for (const auto & i : m) {
            av_dist += (i[0]-cx) * (i[0]-cx) + (i[1]-cy) * (i[1]-cy);
        }
        av_dist /= static_cast<int>(m.size());
        av_dist = std::sqrt(av_dist);
        double s;
        if (av_dist < 1e-12) {
            s = 1.;
        } else {
            s = std::sqrt(2.0) / av_dist;
        }
        const cv::Matx33d norm(s, 0, -s*cx,
                         0, s, -s*cy,
                         0, 0, 1.0);
