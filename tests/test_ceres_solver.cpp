...
TEST (CeresSolver, HelloWorld1) {
    ...
    // ceres вычисляет производную с помощью дуальных чисел, но несмотря на это небольшая погрешность все равно есть,
    // она связана с техническими ограничениями железа и вычисления чисел с плавающей точкой
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        T b = (T) line[1];
        T c = (T) line[2];
        residual[0] = (a*samplePoint[0] + b*samplePoint[1] + c) / ceres::sqrt(a*a + b*b);
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // причина заключается в том что мы не нормируем коэфициенты прямой, из-за этого одна и та же прямая не совпадает
        // как это можно решить? придумайте хотя бы два способа:
        // - пост-обработкой - как-то поправив параметры прямой перед сверкой (при этом не меняя ее положение в пространстве)
        // отнормировать прямые после обработки(приводим нашу прямую к эталонной уравнивая коэффициент c)
        ...
        // можно понизить степень свободы, сделав один из параметров зависимым от другого, таким образом мы зафиксируем масштаб
        // например можно допустить инвариант что a^2 + b^2 = 1, и решать задачу например для параметров b и c, а a выражать через b
    }

    // Оцениваем качество идеальной прямой
    double inliers_fraction, mse;
    evaluateLine(points, ideal_line, sigma, inliers_fraction, mse);
    ASSERT_GT(inliers_fraction, 0.99 - outliers_fraction); //почему эта проверка падает? как поправить?
    // эта проверка падает на тестах с outliers потому что они не входят в inliers_fraction. чтобы подправить нужно вычитать процент outliers
    ASSERT_LT(mse, 1.1 * sigma * sigma); // почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    // аналогично с предыдущим проверка падает на тестах с outliers, потому что квадратичная ошибка у шума может быть очень большой и они попадают в mse
    // Ошибка в методе evaluateLine. Решение: брать модуль расстояния
    ...
    if (outliers_fraction == 0 || use_huber) {
        // почему они падают? в каких тестах? поправьте (в т.ч. подобно тому как было с ослаблением порога выше)
        // аналогично
